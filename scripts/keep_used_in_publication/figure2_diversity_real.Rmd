...
cor.test(div_tog_bac$max_tax, div_tog_bac$load_bac, method = "spearman")
...
Shannon_div_plot_bac<-ggplot(data=div_tog, aes(x=load_bac, y=shannon)) + 
  ...
  geom_smooth(method='lm', color='gray50', level=0) +
  theme_light() + 
  theme( panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  xlab("Total Load") +
  #xlab(expression(paste(' Load (log'[10],"(Total Microb. Cov./Plant Cov.))", sep=""))) +
...
max_plot_bac<-ggplot(data=div_tog, aes(x=(load_bac), y=max_tax_bac)) + 
  ...
  #geom_smooth(method='lm', color='red', level=0) +
  geom_line(aes(y = predict(load_max_bac, div_tog_bac)), colour = "gray50") +
  theme_light() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  xlab("Total Load") +
  #xlab(expression(paste(' Load (log'[10],"(Total Microb. Cov./Plant Cov.))", sep=""))) +
...
```
########WEIRE
## THESE WERE ATTEMPTS TO CONTROL COVERAGE 
...
shannon_div <- vegan::diversity((otu_table(t(phy_deseq_stab))), index = "simpson")
simpson_div <- vegan::diversity(g_s_otus_filter_bacteria, index="simpson")
plot(sample_data(phy_deseq_stab)$load, shannon_div)

ot = otu_table(phy_deseq_stab)
max_tax_bac=apply((otu_table(phy_deseq_stab)), 1, max)
```
...
g_s_all_info = bind_cols(country = g_s_country, pop = g_s_pop, load_bac = g_s_load_bacteria, load_fungi = g_s_load_fungi, load_oom = g_s_load_oom)
