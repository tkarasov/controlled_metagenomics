...
load_hist = data.frame(pseud = log10(g_s$Pseudomonadaceae + .0001), sphingo = log10(g_s$Sphingomonadaceae + 0.0001))
...
both_plot = ggplot(data=load_hist, aes(x=value, fill=variable, color = variable)) + 
  ...
  coord_flip() +
  theme( axis.title = element_blank(), legend.position = "Right", axis.text = element_blank(), axis.line = element_blank(), axis.ticks = element_blank(), plot.margin = unit(c(0, 0, 0, 0), "cm"), panel.border = element_blank()) +
...
pdf("~/Dropbox/controlled_metagenomics/results_figures/field_lab_comparison.pdf", family = "ArialMT", height = 2, width = 3, useDingbats = FALSE)
...
pdf("~/Dropbox/controlled_metagenomics/results_figures/hpa_field_lab_comparison.pdf", family = "ArialMT", height = 2, width = 3, useDingbats = FALSE)
plot_grid(meta_plot, both_plot, align = "hv", rel_widths = c(1, 1))
dev.off()
